...
<div class="blog-header">
    <h1 class="header">Welcome to Blogger Webpage</h1>
</div>
@endsection
    ...
    <div class="container">
        @foreach($posts as $post)
            @php
                ...
                $imagePath = "/imgs/";
                $imagePath = "$imagePath$post->img";
                $content = substr(strip_tags($post->content), 0, 480);
            @endphp
            
            <a href={{ $post_url }}><img src={{ $imagePath }} alt="post_img" width="700" height="400"></a>
            <h3><a href="{{ $post_url }}" class="btn"><strong>{{ $post->title }}</strong></a></h3>
            <div>
                <span>Author: Alaa  |  </span>
                ...
                <span>
                    ...
                    {{count(Comment::where("post_id",$post->id)->get())}}
                </span>
            </div>
            <p>{{$content}}...</p>
